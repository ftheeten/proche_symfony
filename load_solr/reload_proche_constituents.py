...
h = httplib2.Http()
h.add_credentials('proche_admin', 'pr0#µeb202E') 
SOLR_URL='https://proche.africamuseum.be/solradmin/solr/proche-dev_dec2024/' 
...
AS  \
(  \
SELECT a.ID as ObjectID, c_phys.DisplayName, c_phys.BeginDate, c_phys.EndDate, Role , ConstituentTypeID, c_phys.ConstituentID , BeginDateISO, EndDateISO \
FROM [TMS].[dbo].[vConXrefsAll]  \
INNER JOIN c_phys \
...
AS \
(  \
SELECT a.ID as ObjectID, 'NOT_DISPLAYED' DisplayName, NULL BeginDate, NULL EndDate, Role , ConstituentTypeID, c_phys.ConstituentID, BeginDateISO, EndDateISO \
FROM [TMS].[dbo].[vConXrefsAll]  \
INNER JOIN c_phys \
ON vConXrefsAll.ConstituentID = c_phys.ConstituentID  \
INNER JOIN a ON  \
...
AS \
(  \
SELECT a.ID as ObjectID, c_mor.DisplayName, c_mor.BeginDate, c_mor.EndDate, Role , ConstituentTypeID, c_mor.ConstituentID, BeginDateISO, EndDateISO \
FROM [TMS].[dbo].[vConXrefsAll]  \
INNER JOIN c_mor \
ON vConXrefsAll.ConstituentID = c_mor.ConstituentID  \
INNER JOIN a ON  \
...
f as \
( \
SELECT e.*, TextEntryHTML FROM e LEFT JOIN [TMS].[dbo].[TextEntries] txt ON ConstituentID=txt.ID AND TableID=23 AND TextTypeID=97 AND TextStatusID=2 \
) \
SELECT * FROM f"
    data=pnd.read_sql(sql=sql, con=conn)
    return data
 
def get_constituents_alt_names( conn, p_pnd_cons):
    global global_db
    ...
    data["v_cons_all_names_implode"]=data.apply(lambda x: "; ".join(x['v_cons_all_names']), axis=1)
...
def add_const(doc, field_name, values):
    ...
    elif field_name.endswith("_bio") or  field_name.endswith("_alt_names") :
        doc[field_name]= ""        
    return doc
...
def handle_bio(p_arr_bio,  p_arr_alt_name, bio,  alt):
    if not bio is None:
        p_arr_bio.append(bio)
    else:
        p_arr_bio.append("''")
    if not alt is None:
        p_arr_alt_name.append(alt)
    else:
        p_arr_alt_name.append("''")
...
def handle_constituents(pnd_cons, obj_id, pnd_translations):
    ...
    const_general_bio=[]
    const_date_general_bio=[]
    const_collector_bio=[]
    const_depositor_bio=[]
    const_depot_bio=[]
    const_donor_bio=[]
    const_excavation_by_bio=[]
    const_exchange_bio=[]
    const_field_collector_bio=[]
    const_first_owner_bio=[]
    const_intermediary_bio=[]
    const_legator_bio=[]
    const_lender_bio=[]
    const_maker_creator_bio=[]
    const_mission_bio=[]
    const_owner_bio=[]
    const_previous_owner_bio=[]
    const_transfer_bio=[]
    const_unknown_bio=[]
    const_vendor_bio=[]
    
    const_date_collector_bio=[]
    const_date_depositor_bio=[]
    const_date_depot_bio=[]
    const_date_donor_bio=[]
    const_date_excavation_by_bio=[]
    const_date_exchange_bio=[]
    const_date_field_collector_bio=[]
    const_date_first_owner_bio=[]
    const_date_intermediary_bio=[]
    const_date_legator_bio=[]
    const_date_lender_bio=[]
    const_date_maker_creator_bio=[]
    const_date_mission_bio=[]
    const_date_owner_bio=[]
    const_date_previous_owner_bio=[]
    const_date_transfer_bio=[]
    const_date_unknown_bio=[]
    const_date_vendor_bio=[]
    
    const_trans_general_bio=[]
    const_trans_maker_creator_bio=[]
    
    const_general_alt_names=[]
    const_date_general_alt_names=[]
    const_collector_alt_names=[]
    const_depositor_alt_names=[]
    const_depot_alt_names=[]
    const_donor_alt_names=[]
    const_excavation_by_alt_names=[]
    const_exchange_alt_names=[]
    const_field_collector_alt_names=[]
    const_first_owner_alt_names=[]
    const_intermediary_alt_names=[]
    const_legator_alt_names=[]
    const_lender_alt_names=[]
    const_maker_creator_alt_names=[]
    const_mission_alt_names=[]
    const_owner_alt_names=[]
    const_previous_owner_alt_names=[]
    const_transfer_alt_names=[]
    const_unknown_alt_names=[]
    const_vendor_alt_names=[]
    
    const_date_collector_alt_names=[]
    const_date_depositor_alt_names=[]
    const_date_depot_alt_names=[]
    const_date_donor_alt_names=[]
    const_date_excavation_by_alt_names=[]
    const_date_exchange_alt_names=[]
    const_date_field_collector_alt_names=[]
    const_date_first_owner_alt_names=[]
    const_date_intermediary_alt_names=[]
    const_date_legator_alt_names=[]
    const_date_lender_alt_names=[]
    const_date_maker_creator_alt_names=[]
    const_date_mission_alt_names=[]
    const_date_owner_alt_names=[]
    const_date_previous_owner_alt_names=[]
    const_date_transfer_alt_names=[]
    const_date_unknown_alt_names=[]
    const_date_vendor_alt_names=[]
    
    const_trans_general_alt_names=[]
    const_trans_maker_creator_alt_names=[]
    ...
    for i, row in filtered.iterrows():
        ...
        bio=None
        if not row["TextEntryHTML"] is None:
            bio=row["TextEntryHTML"]
            
        v_cons_all_names_implode=None
        if not row["v_cons_all_names_implode"] is None:
            v_cons_all_names_implode=row["v_cons_all_names_implode"]    
        #print(name)
        
        dates=format_year_life(row)
        #print("===>")
        #print(dates)
        ...
        if role=="collector":
            ...
            handle_bio(const_collector_bio,  const_collector_alt_names, bio, v_cons_all_names_implode)
            
            #const_general.append(name)
        elif role=="depositor":
            const_depositor.append(name)
            const_date_depositor.append(name_with_date)
            handle_bio(const_date_depositor_bio,  const_date_depositor_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)             
        elif role=="depot":
            const_depot.append(name)
            const_date_depot.append(name_with_date)
            handle_bio(const_depot_bio,  const_depot_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)  
        elif role=="donor":
            const_donor.append(name)
            const_date_donor.append(name_with_date)
            handle_bio(const_donor_bio,  const_donor_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)              
        elif role=="excavation by":
            const_excavation_by.append(name)
            const_date_excavation_by.append(name_with_date)
            handle_bio(const_excavation_by_bio,  const_excavation_by_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        elif role=="exchange":
            const_exchange.append(name)
            const_date_exchange.append(name_with_date)
            handle_bio(const_exchange_bio,  const_exchange_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)            
        elif role=="field collector":
            const_field_collector.append(name)
            const_date_collector.append(name_with_date)
            handle_bio(const_field_collector_bio,  const_field_collector_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        elif role=="first owner":
            const_first_owner.append(name)
            const_date_first_owner.append(name_with_date)
            handle_bio(const_first_owner_bio,  const_first_owner_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        elif role=="intermediary":
            const_intermediary.append(name)
            const_date_intermediary.append(name_with_date)
            handle_bio(const_intermediary_bio,  const_intermediary_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        elif role=="legator":
            const_legator.append(name)
            const_date_legator.append(name_with_date)
            handle_bio(const_legator_bio,  const_legator_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        elif role=="lender":
            const_lender.append(name)
            const_date_lender.append(name_with_date)
            handle_bio(const_lender_bio,  const_lender_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
            ...
            handle_bio(const_maker_creator_bio,  const_maker_creator_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        elif role=="mission":
            const_mission.append(name)
            const_date_mission.append(name_with_date)
            handle_bio(const_mission_bio,  const_mission_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        elif role=="owner":
            const_owner.append(name)
            const_date_owner.append(name_with_date)
            handle_bio(const_owner_bio,  const_owner_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        elif role=="previous owner":
            const_previous_owner.append(name)
            const_date_previous_owner.append(name_with_date)
            handle_bio(const_previous_owner_bio,  const_previous_owner_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        elif role=="transfer":
            const_transfer.append(name)
            const_date_transfer.append(name_with_date)
            handle_bio(const_transfer_bio,  const_transfer_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        elif role=="unknown":
            const_unknown.append(name)
            const_date_unknown.append(name_with_date)
            handle_bio(const_unknown_bio,  const_unknown_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        elif role=="vendor":
            const_vendor.append(name)
            const_date_vendor.append(name_with_date)
            handle_bio(const_vendor_bio,  const_vendor_alt_names, bio, v_cons_all_names_implode)
            #const_general.append(name)
        const_general.append(name)
        ...
        #bio
        dict_c=add_const(dict_c, "const_collector_bio", const_collector_bio)
        dict_c=add_const(dict_c, "const_depositor_bio",const_depositor_bio)
        dict_c=add_const(dict_c, "const_depot_bio",const_depot_bio)
        dict_c=add_const(dict_c, "const_donor_bio",const_donor_bio)
        dict_c=add_const(dict_c, "const_excavation_by_bio",const_excavation_by_bio)
        dict_c=add_const(dict_c, "const_exchange_bio",const_exchange_bio)
        dict_c=add_const(dict_c, "const_field_collector_bio",const_field_collector_bio)
        dict_c=add_const(dict_c, "const_first_owner_bio",const_first_owner_bio)
        dict_c=add_const(dict_c, "const_intermediary_bio",const_intermediary_bio)
        dict_c=add_const(dict_c, "const_legator_bio",const_legator_bio)
        dict_c=add_const(dict_c, "const_lender_bio",const_lender_bio)
        dict_c=add_const(dict_c, "const_maker_creator_bio",const_maker_creator_bio)
        dict_c=add_const(dict_c, "const_mission_bio",const_mission_bio)
        dict_c=add_const(dict_c, "const_owner_bio",const_owner_bio)
        dict_c=add_const(dict_c, "const_previous_owner_bio",const_previous_owner_bio)
        dict_c=add_const(dict_c, "const_transfer_bio",const_transfer_bio)
        dict_c=add_const(dict_c, "const_unknown_bio",const_unknown_bio)
        dict_c=add_const(dict_c, "const_vendor_bio",const_vendor_bio)
        
        dict_c=add_const(dict_c, "const_date_collector_bio", const_date_collector_bio)
        dict_c=add_const(dict_c, "const_date_depositor_bio",const_date_depositor_bio)
        dict_c=add_const(dict_c, "const_date_depot_bio",const_date_depot_bio)
        dict_c=add_const(dict_c, "const_date_donor_bio",const_date_donor_bio)
        dict_c=add_const(dict_c, "const_date_excavation_by_bio",const_date_excavation_by_bio)
        dict_c=add_const(dict_c, "const_date_exchange_bio",const_date_exchange_bio)
        dict_c=add_const(dict_c, "const_date_field_collector_bio",const_date_field_collector_bio)
        dict_c=add_const(dict_c, "const_date_first_owner_bio",const_date_first_owner_bio)
        dict_c=add_const(dict_c, "const_date_intermediary_bio",const_date_intermediary_bio)
        dict_c=add_const(dict_c, "const_date_legator_bio",const_date_legator_bio)
        dict_c=add_const(dict_c, "const_date_lender_bio",const_date_lender_bio)
        dict_c=add_const(dict_c, "const_date_maker_creator_bio",const_date_maker_creator_bio)
        dict_c=add_const(dict_c, "const_date_mission_bio",const_date_mission_bio)
        dict_c=add_const(dict_c, "const_date_owner_bio",const_date_owner_bio)
        dict_c=add_const(dict_c, "const_date_previous_owner_bio",const_date_previous_owner_bio)
        dict_c=add_const(dict_c, "const_date_transfer_bio",const_date_transfer_bio)
        dict_c=add_const(dict_c, "const_date_unknown_bio",const_date_unknown_bio)
        dict_c=add_const(dict_c, "const_date_vendor_bio",const_date_vendor_bio)
        
        #alt names
        
        dict_c=add_const(dict_c, "const_collector_alt_names", const_collector_alt_names)
        dict_c=add_const(dict_c, "const_depositor_alt_names",const_depositor_alt_names)
        dict_c=add_const(dict_c, "const_depot_alt_names",const_depot_alt_names)
        dict_c=add_const(dict_c, "const_donor_alt_names",const_donor_alt_names)
        dict_c=add_const(dict_c, "const_excavation_by_alt_names",const_excavation_by_alt_names)
        dict_c=add_const(dict_c, "const_exchange_alt_names",const_exchange_alt_names)
        dict_c=add_const(dict_c, "const_field_collector_alt_names",const_field_collector_alt_names)
        dict_c=add_const(dict_c, "const_first_owner_alt_names",const_first_owner_alt_names)
        dict_c=add_const(dict_c, "const_intermediary_alt_names",const_intermediary_alt_names)
        dict_c=add_const(dict_c, "const_legator_alt_names",const_legator_alt_names)
        dict_c=add_const(dict_c, "const_lender_alt_names",const_lender_alt_names)
        dict_c=add_const(dict_c, "const_maker_creator_alt_names",const_maker_creator_alt_names)
        dict_c=add_const(dict_c, "const_mission_alt_names",const_mission_alt_names)
        dict_c=add_const(dict_c, "const_owner_alt_names",const_owner_alt_names)
        dict_c=add_const(dict_c, "const_previous_owner_alt_names",const_previous_owner_alt_names)
        dict_c=add_const(dict_c, "const_transfer_alt_names",const_transfer_alt_names)
        dict_c=add_const(dict_c, "const_unknown_alt_names",const_unknown_alt_names)
        dict_c=add_const(dict_c, "const_vendor_alt_names",const_vendor_alt_names)
        
        dict_c=add_const(dict_c, "const_date_collector_alt_names", const_date_collector_alt_names)
        dict_c=add_const(dict_c, "const_date_depositor_alt_names",const_date_depositor_alt_names)
        dict_c=add_const(dict_c, "const_date_depot_alt_names",const_date_depot_alt_names)
        dict_c=add_const(dict_c, "const_date_donor_alt_names",const_date_donor_alt_names)
        dict_c=add_const(dict_c, "const_date_excavation_by_alt_names",const_date_excavation_by_alt_names)
        dict_c=add_const(dict_c, "const_date_exchange_alt_names",const_date_exchange_alt_names)
        dict_c=add_const(dict_c, "const_date_field_collector_alt_names",const_date_field_collector_alt_names)
        dict_c=add_const(dict_c, "const_date_first_owner_alt_names",const_date_first_owner_alt_names)
        dict_c=add_const(dict_c, "const_date_intermediary_alt_names",const_date_intermediary_alt_names)
        dict_c=add_const(dict_c, "const_date_legator_alt_names",const_date_legator_alt_names)
        dict_c=add_const(dict_c, "const_date_lender_alt_names",const_date_lender_alt_names)
        dict_c=add_const(dict_c, "const_date_maker_creator_alt_names",const_date_maker_creator_alt_names)
        dict_c=add_const(dict_c, "const_date_mission_alt_names",const_date_mission_alt_names)
        dict_c=add_const(dict_c, "const_date_owner_alt_names",const_date_owner_alt_names)
        dict_c=add_const(dict_c, "const_date_previous_owner_alt_names",const_date_previous_owner_alt_names)
        dict_c=add_const(dict_c, "const_date_transfer_alt_names",const_date_transfer_alt_names)
        dict_c=add_const(dict_c, "const_date_unknown_alt_names",const_date_unknown_alt_names)
        dict_c=add_const(dict_c, "const_date_vendor_alt_names",const_date_vendor_alt_names)
...
try:  
    ...
    pnd_constituents=get_constituents_alt_names(cn, pnd_constituents)
